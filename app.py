...
def load_proxy_config():
    ...
    try:
        with open(PROXY_NODES_FILE, "r", encoding="utf-8") as f:
            ...
            try:
                before = json.dumps(
                    data.get("nodes", []), ensure_ascii=False, sort_keys=True
                )
                assign_local_ports(data.get("nodes", []))
                after = json.dumps(
                    data.get("nodes", []), ensure_ascii=False, sort_keys=True
                )
                if before != after:
                    save_proxy_config(data)
            except ValueError:
                pass
            return data
    except Exception:
...
def assign_local_ports(nodes, existing_nodes=None):
    used_ports = set()
    for n in existing_nodes or []:
        port = normalize_proxy_port(n.get("local_port"))
        if port:
            used_ports.add(port)

    next_port = max(PROXY_BASE_PORT, 1024)
    for node in nodes:
        locked = node.get("local_port_locked")
        port = normalize_proxy_port(node.get("local_port"))
        if port and port not in used_ports:
            used_ports.add(port)
            node["local_port"] = port
            if locked is None:
                node["local_port_locked"] = True
            continue
        ...
        node["local_port"] = next_port
        node["local_port_locked"] = False
...
def get_accounts():
    ...
    for acc in accounts:
        ...
        result_acc = {
            ...
            "keepalive_connect_timeout": acc.get("keepalive_connect_timeout", 10),
            "keepalive_request_timeout": acc.get("keepalive_request_timeout", 15),
            "keepalive_sse_read_timeout": acc.get("keepalive_sse_read_timeout", 90),
            "keepalive_retry_total": acc.get("keepalive_retry_total", 2),
            "keepalive_retry_backoff": acc.get("keepalive_retry_backoff", 2),
            "keepalive_fail_threshold": acc.get("keepalive_fail_threshold", 3),
            "keepalive_cooldown_seconds": acc.get("keepalive_cooldown_seconds", 60),
            "keepalive_stagnation_kill_count": acc.get(
                "keepalive_stagnation_kill_count", 2
            ),
            "keepalive_stagnation_min_increase": acc.get(
                "keepalive_stagnation_min_increase", 0.0
            ),
...
def add_account():
    ...
    keepalive_connect_timeout = data.get("keepalive_connect_timeout", 10)
    keepalive_request_timeout = data.get("keepalive_request_timeout", 15)
    keepalive_sse_read_timeout = data.get("keepalive_sse_read_timeout", 90)
    keepalive_retry_total = data.get("keepalive_retry_total", 2)
    keepalive_retry_backoff = data.get("keepalive_retry_backoff", 2)
    keepalive_fail_threshold = data.get("keepalive_fail_threshold", 3)
    keepalive_cooldown_seconds = data.get("keepalive_cooldown_seconds", 60)
    keepalive_stagnation_kill_count = data.get("keepalive_stagnation_kill_count", 2)
    keepalive_stagnation_min_increase = data.get(
        "keepalive_stagnation_min_increase", 0.0
    )
    ...
    try:
        keepalive_wait_seconds = int(keepalive_wait_seconds)
        keepalive_start_interval = int(keepalive_start_interval)
        keepalive_connect_timeout = int(keepalive_connect_timeout)
        keepalive_request_timeout = int(keepalive_request_timeout)
        keepalive_sse_read_timeout = int(keepalive_sse_read_timeout)
        keepalive_retry_total = int(keepalive_retry_total)
        keepalive_retry_backoff = float(keepalive_retry_backoff)
        keepalive_fail_threshold = int(keepalive_fail_threshold)
        keepalive_cooldown_seconds = int(keepalive_cooldown_seconds)
        keepalive_stagnation_kill_count = int(keepalive_stagnation_kill_count)
        keepalive_stagnation_min_increase = float(keepalive_stagnation_min_increase)
    except (TypeError, ValueError):
        return jsonify({"error": "KeepAlive 参数必须是数字"}), 400
    ...
    new_account = {
        ...
        "keepalive_connect_timeout": keepalive_connect_timeout,
        "keepalive_request_timeout": keepalive_request_timeout,
        "keepalive_sse_read_timeout": keepalive_sse_read_timeout,
        "keepalive_retry_total": keepalive_retry_total,
        "keepalive_retry_backoff": keepalive_retry_backoff,
        "keepalive_fail_threshold": keepalive_fail_threshold,
        "keepalive_cooldown_seconds": keepalive_cooldown_seconds,
        "keepalive_stagnation_kill_count": keepalive_stagnation_kill_count,
        "keepalive_stagnation_min_increase": keepalive_stagnation_min_increase,
        "enabled": True,
...
def update_account(server_id):
    ...
    for acc in accounts:
        if acc.get("server_id") == server_id:
            ...
            if "keepalive_connect_timeout" in data:
                try:
                    acc["keepalive_connect_timeout"] = int(
                        data["keepalive_connect_timeout"]
                    )
                except (TypeError, ValueError):
                    pass
            if "keepalive_request_timeout" in data:
                try:
                    acc["keepalive_request_timeout"] = int(
                        data["keepalive_request_timeout"]
                    )
                except (TypeError, ValueError):
                    pass
            if "keepalive_sse_read_timeout" in data:
                try:
                    acc["keepalive_sse_read_timeout"] = int(
                        data["keepalive_sse_read_timeout"]
                    )
                except (TypeError, ValueError):
                    pass
            if "keepalive_retry_total" in data:
                try:
                    acc["keepalive_retry_total"] = int(data["keepalive_retry_total"])
                except (TypeError, ValueError):
                    pass
            if "keepalive_retry_backoff" in data:
                try:
                    acc["keepalive_retry_backoff"] = float(
                        data["keepalive_retry_backoff"]
                    )
                except (TypeError, ValueError):
                    pass
            if "keepalive_fail_threshold" in data:
                try:
                    acc["keepalive_fail_threshold"] = int(
                        data["keepalive_fail_threshold"]
                    )
                except (TypeError, ValueError):
                    pass
            if "keepalive_cooldown_seconds" in data:
                try:
                    acc["keepalive_cooldown_seconds"] = int(
                        data["keepalive_cooldown_seconds"]
                    )
                except (TypeError, ValueError):
                    pass
            if "keepalive_stagnation_kill_count" in data:
                try:
                    acc["keepalive_stagnation_kill_count"] = int(
                        data["keepalive_stagnation_kill_count"]
                    )
                except (TypeError, ValueError):
                    pass
            if "keepalive_stagnation_min_increase" in data:
                try:
                    acc["keepalive_stagnation_min_increase"] = float(
                        data["keepalive_stagnation_min_increase"]
                    )
                except (TypeError, ValueError):
                    pass
            if "enabled" in data:
            ...
            if acc.get("server_id") == server_id:
                ...
                if platform == "keepalive":
                    return jsonify(
                        {
                            "success": False,
                            "message": "KeepAlive 为实时模式，不支持手动续期",
                        }
                    ), 400
                try:
                    if token:
...
def add_proxy_link():
    ...
    try:
        assign_local_ports([node], existing_nodes=config.get("nodes", []))
...
def sync_proxy_subscription():
    ...
    try:
        if mode == "append":
            assign_local_ports(nodes, existing_nodes=config.get("nodes", []))
            config["nodes"] = config.get("nodes", []) + nodes
        else:
            assign_local_ports(nodes)
            config["nodes"] = nodes
    except ValueError as e:
        return jsonify({"success": False, "error": str(e)}), 400
...
def update_proxy_node(node_id):
    ...
    if local_port in (None, ""):
        ...
        node.pop("local_port_locked", None)
    else:
        try:
...
if __name__ == "__main__":
    ...
    apply_proxy_config()
    bg = start_background_task(silent=True)
    if bg is None:
        print("检测到已有后台实例，当前进程仅启动 Web，后台等待锁释放")

        def wait_for_background():
            while True:
                time.sleep(30)
                bg_started = start_background_task(silent=True)
                if bg_started:
                    print("后台实例已接管")
                    break

        threading.Thread(target=wait_for_background, daemon=True).start()
