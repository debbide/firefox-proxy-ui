...
const reserveAvailablePort = async (host) => {
    return await new Promise((resolve, reject) => {
        ...
        server.listen({ host, port: 0 }, () => {
            ...
            server.close(() => {
                if (!port) return reject(new Error('Failed to allocate port'));
                resolve(port);
            });
        });
    });
...
const main = async () => {
    try {
        ...
        if (action === 'test') {
            ...
            const listenHost = process.env.PROXY_LISTEN || '127.0.0.1';
            try {
                node.local_port = await reserveAvailablePort(listenHost);
            } catch (e) {
                return fail(e && e.message ? e.message : 'Failed to allocate local port');
            }
            service.setNodes([node]);
            await service.start();
            await sleep(800);
            if (!service.proxyProcess || service.proxyProcess.exitCode !== null) {
                service.stop();
                return fail('Proxy process failed to start');
            }
